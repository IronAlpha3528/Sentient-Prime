...
graph TB
    subgraph "Data Sources"
        A1[Network Telemetry<br/>Suricata, Zeek, NetFlow]
        A2[Identity Telemetry<br/>Active Directory, VPN]
        A3[Endpoint Telemetry<br/>Sysmon, auditd]
        A4[OT/ICS Telemetry<br/>Modbus, Historian]
    end
    
    subgraph "SIEM Layer"
        B[Wazuh + Elasticsearch<br/>Normalization & Baseline]
    end
    
    subgraph "Specialist Detection Layer"
        C1[Network Detector<br/>LightGBM]
        C2[Identity/UEBA Detector<br/>Isolation Forest]
        C3[Endpoint Detector<br/>LightGBM + Sigma]
        C4[OT Detector<br/>Isolation Forest]
    end
    
    subgraph "Evidence Fusion Framework"
        D1[Common Evidence Object]
        D2[Cyber Entity Graph<br/>NetworkX MultiDiGraph]
        D3[LightGBM Meta-Classifier]
    end
    
    subgraph "Knowledge Retrieval"
        E1[MITRE ATT&CK<br/>STIX 2.1 Knowledge Graph]
        E2[FAISS Vector Index<br/>Semantic Search]
        E3[Hybrid Graph-RAG<br/>Retriever]
    end
    
    subgraph "Constrained AI Reasoning"
        F1[Analysis Agent<br/>Gemini 3.1 Pro]
        F2[Critique Agent<br/>Gemini 3.1 Pro]
        F3[Action Agent<br/>Gemini 3.1 Pro]
    end
    
    subgraph "Adaptive Deception"
        G1[Hypothesis Testing Logic]
        G2[Decoy Deployment<br/>Honeytokens, Conpot]
        G3[Interaction Monitor]
    end
    
    subgraph "Deterministic Control"
        H1[Risk Engine<br/>Python/NumPy]
        H2[Dry-Run Simulator]
        H3[Policy Gate]
    end
    
    subgraph "Orchestration & Response"
        I1[SOAR Playbooks<br/>Action Allowlist]
        I2[EDR/Firewall/IAM APIs]
    end
    
    subgraph "Observability"
        J1[Outcome Monitor]
        J2[SHA-256 Audit Ledger]
        J3[Dashboard<br/>React SPA + FastAPI]
    end
    ...
    D3 -.Moderate Confidence.-> G1
    G1 --> G2
    G2 --> G3
    G3 -.Decoy Interaction.-> D1
