...
const items = [
  ...
  { label: 'Approval Queue', path: '/approval-queue', icon: 'M9 11l3 3L22 4M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11', badge: true },
      ...
      <nav className="nav">
        {items.map((item) => (
          <NavLink key={item.label} to={item.path} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
            <svg className="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={item.icon} /></svg>
            <span className="nav-label" style={{ flex: 1 }}>{item.label}</span>
            {item.badge && pendingCount > 0 && (
              <span className="badge danger pulse" style={{ fontSize: 10, padding: '1px 7px', marginLeft: 4 }}>{pendingCount}</span>
            )}
          </NavLink>
        ))}
      </nav>
